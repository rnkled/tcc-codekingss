...
const Chat: React.FC = () => {
  ...
  function getMessage() {
    if (user.role === "user") {
      const onValueChange = database()
        .ref(`/chats/${route.params.id_professional}/${user._id}/messages/`)
        .orderByKey()
        .on("value", (snapshot) => {
          var messagesArr = [];
          snapshot &&
    ...
    if (user.role === "professional") {
      const onValueChange = database()
        .ref(`/chats/${user._id}/${route.params.id_pacient}/messages/`)
        .orderByKey()
        .on("value", (snapshot) => {
          var messagesArr = [];
          snapshot &&
  ...
  async function handleChatMessage() {
    if (message) {
      if (user.role === "user") {
        const newReferenceMessage = database()
          .ref(`/chats/${route.params.id_professional}/${user._id}/messages`)
          .push();
        const messageObject = {
          id_user: user._id,
          id: Date.now(),
          message,
          sent: moment(new Date().toLocaleTimeString("pt-BR"), "HH:mm:ss")
            .add(3, "hours")
  ...
  return (
    <Background style={styles.container}>
      <Header
        buttonLeft={{ label: "Voltar", onPress: goBack }}
        buttonRight={{ label: "Opções", onPress: goBack }}
        titlePage={route.params.name}
        fontSize={24}
      />
        ...
        <AutoScrollFlatList
          ...
          renderItem={({ item }) => (
            <View
              style={{
                alignItems:
                  item.user_type === user.role && item.id_user === user._id
                    ? "flex-end"
                    : "flex-start",
              }}
            >
      ...
      <View style={styles.contentInput}>
        <InputTextChat
          onPressButton={handleChatMessage}
          text={message}
