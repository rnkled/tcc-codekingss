...
const CalendarComponent: React.FC = () => {
  ...
  function updateDataUser() {
    setLoading(true);
    api
      .get("/appointment/userlist/" + user._id)
      .then((response) => {
        const APIData = response.data.appointments;
        let dataObject = {};
        APIData &&
        ...
        {loading ? (
          ...
          <>
              ...
              <Calendar
                ...
                markedDates={{
                  ...
                  [`${selectedDay.dateString}`]: {
                    selected: true,
                    selectedColor: theme.primaryVariant55,
                  },
                }}
                onDayPress={(day) => {
                  setSelectedDay(day);
            ...
            <View style={styles.contentSecondary}>
              {apiData &&
              apiData[
                selectedDay?.dateString.split("-").reverse().join("-")
              ] ? (
                <>
                  <Text style={styles.titleAppointments}>
                    Agendamentos de {getDate(new Date(selectedDay.dateString))}
                  </Text>
                  {apiData[
                    selectedDay.dateString.split("-").reverse().join("-")
                  ] &&
                    apiData[
                      selectedDay.dateString.split("-").reverse().join("-")
                    ].map((item: appointmentInterface, index: number) => (
                      <AppointmentItem
                        key={index}
                        item={item}
                        updateData={updateData}
                        goToAppointment={goToAppointment}
                        manage={user.role === "professional" ? true : false}
                        date={selectedDay.dateString}
                      />
                    ))}
                </>
              ...
              {user.role === "professional" && (
                ...
                >
                  ...
                  />
                  <Text style={styles.textAddAppointment}>
